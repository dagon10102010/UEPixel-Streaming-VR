...
if (config.UseFrontend) {
	...
	var httpPort = process.env.PORT || 9998;
	var httpsPort = 443;
}
...
app.post('/start',bodyParser.urlencoded({ extended: false }), function(req, res){
	if(req.body.username =='trung' && req.body.password=="145632"){
		exec('runapp.bat '+req.body.project)
		res.send('done')
	}
	else
		res.redirect('/start');
...
if(config.EnableWebserver) {
	...
	app.use('/start', express.static(path.join(__dirname, '/start.html')))
}
...
playerServer.on('connection', function (ws, req) {
	...
	function sendPlayersCount() {
		const msg = { type: 'playerCount', count: players.size,list:[] }
		for (let p of players.values()) {
			msg.list.push(p.name)
		}
		let playerCountMsg = JSON.stringify(msg);
	...
	ws.on('message', function (msg) {
		...
		if (msg.type == 'offer') {
			...
			players.get(playerId).name = msg.name
			sendPlayersCount();
